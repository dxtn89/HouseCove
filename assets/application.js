$(document).ready(function () {
  ...
  let
   ...
   let currencyPicker = {
    ...
    onCurrencyChanged: function (e) {
      ...
      if ($otherPickers.length > 0) { //if there are other pickers, set val to the new currency val
        $otherPickers.val(newCurrency);
      }
    },
    setCurrencyText: function (newCurrency = Currency.currentCurrency) {
      //element variables have the dollar sign
      //check if we have the elements to be swapped out on the page
      let
        $activeCurrency = $(activeCurrencySelector),
        $currencyNote = $(currencyNoteSelector)

      //if true, set the text to the right  
    ...
    onMoneySpanAdded: function () {
      //this will be called when new money span gets added to the document (via the ajax cart.. etc)
      Currency.convertAll(shopCurrency, Currency.currentCurrency);
      currencyPicker.setCurrencyText();
    },
    init: function () {
      //if multiple currencies are enabled , initialize all currency picker functions
      if (showMultipleCurrencies !== true) {
        return false; //it wpnt execute anything below this line 
      }
  ...
  let
    ...
    productOptionSelector = addToCartFormSelector + ' [name*=option]'; //look for any name selector that  has option in it
  ...
  let productForm = {
    ...
    getActiveVariant: function ($form) {
      ...
      $.each(formData, function (index, item) { //current index of the loop and current item you're iterating upon
    ...
    validate: function (e, selectedVariant) {
      let
        $form = $(this),
        ...
        formatedVariantPrice,
        priceHtml;

      if (hasVariant) {
        formatedVariantPrice = '$' + (selectedVariant.price / 100).toFixed(2);
        priceHtml = '<span class="money">' + formatedVariantPrice + '</span>';
  ...
  let ajaxify = {
    ...
    onError: function (XMLHttpRequest, textState) {
      let data = XMLHttpRequest.responseJSON;
      alert(data.status + ' - ' + dtat.message + ': ' + data.description);
    },
    onCartButtonClick: function (e) {
  ...
  let lineItem = {
    isInMiniCart: function(element) {
       let 
         $element = $(element),
         $miniCart = $element.closest('#mini-cart'),
         ...
         setup: function($element) {
           let 
             ...
             productSlideshowOptions = {
              ...
              //if there is only one image, we are not gonna go through the hassle of setting up a slideshow
